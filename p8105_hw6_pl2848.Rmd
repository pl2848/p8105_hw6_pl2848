...
To determine which variables should be included into my regression model, I make the corelation matrix and displayed it via heat map.
```{r}
...
Then, the idea is to take out non-significant variables one at a time starting with the highest p-value. Here, I use one function to help me to do it. I think the best_mlr makes sense, because the variables that have low correlations are excluded and the remaining variables are all reasonable and could have potential effect to body weight. So I decided to use the result from backwards elimination as my prediction model.

```{r}
# The best model using backward elimination
best_mlr = step(mult.fit, direction='both')
...
We can see from the violin plot that model 3 has the smallest RMSE, which means that my selected model has the highest accuracy among three tested models.
